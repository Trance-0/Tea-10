...
public class L1706 {
     ...
     * and you have n balls. The box is open on the top and 
     * bottom sides.
     * Each cell in the box has a diagonal board spanning two 
     * corners of the cell that can redirect a ball to the 
     * right or to the left.
     * A board that redirects the ball to the right spans the 
     * top-left corner to the bottom-right corner and is repr-
     * esented in the grid as 1.
     * A board that redirects the ball to the left spans the top-
     * right corner to the bottom-left corner and is represented 
     * in the grid as -1.
     * We drop one ball at the top of each column of the box. 
     * Each ball can get stuck in the box or fall out of the 
     * bottom. A ball gets stuck if it hits a "V" shaped pattern
    ...
    public int[] findBall(int[][] grid) {
        ...
        for(int i=0;i<n;i++){
            ...
            for(int j=0;j<m;j++){
                //test if stucked
                end=start+grid[j][start];
                //if stucked
                if(end<0||end>=n||grid[j][end]!=grid[j][start]){
                    start=-1;
                    break;
                }
                //update steps
